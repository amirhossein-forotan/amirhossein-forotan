...
bool sensorValid[15] = {true, true, true, true, true, true, true, true, true, true, true, true, true, true, true};
...
void setup() {
  ...
  if (digitalRead(PB) == LOW) {
    ...
    for (int i = 0; i < 15; i++) {
      ...
      if (abs(maximums[i] - minimums[i]) < 50) {
        sensorValid[i] = false;
        threshold = 1023;
      }
      ...
      EEPROM.update(30 + i, sensorValid[i]);
    }
    digitalWrite(LED, LOW);
  } else {
    for (int i = 0; i < 15; i++) {
      thresholds[i] = ((EEPROM.read(2 * i + 1)) << 8) + EEPROM.read(2 * i);
      sensorValid[i] = EEPROM.read(30 + i);
    }
  }
}
